...
async function predictNext7Trades() {
    ...
    for (let i = 0; i < 7; i++) {
        const inputTensor = tf.tensor3d([history], [1, SEQUENCE_LENGTH, 2]);
        const prediction = model.predict(inputTensor);
        const predictionArray = await prediction.array();
        const winProb = predictionArray[0][0];

        results.push({ tradeNumber: i + 1, prediction: winProb });

        // Append predicted trade to history
        history.push([1.5, winProb >= 0.5 ? 1 : 0]);
        history = history.slice(1); // Maintain fixed SEQUENCE_LENGTH
    }
